...
namespace Tauron.Features
{
    public sealed class SubscribeFeature : IFeature<SubscribeFeature.State>
    {
        ...
        void IFeature<State>.Init(IFeatureActor<State> actor)
        {
            ...
            actor.Receive<KeyHint>(
                obs => obs.Select(data => data.State.Update(data.Event.Key, refs => refs.Remove(data.Event.Target))));

            actor.Receive<EventSubscribe>(obs => obs.Where(_ => !actor.Sender.IsNobody())
                                                    .Do(m => m.Event.Watch.WhenTrue(
                                                            () => actor.Context.WatchWith(actor.Sender,
                                                                new KeyHint(actor.Sender!, m.Event.Event))))
                                                    .Select(m =>
                                                            {
                                                                var ((_, @event), state, _) = m;

                                                                actor.TellSelf(
                                                                    new InternalEventSubscription(actor.Sender,
                                                                        @event));
                                                                return state.Update(@event,
                                                                    refs => refs.Add(actor.Sender));
                                                            }));
                                                      ...
                                                      .Select(m =>
                                                              {
                                                                  ...
                                                                  return state.Update(eventUnSubscribe.Event,
                                                                      refs => refs.Remove(actor.Sender));
                                                              }));
            ...
            actor.Receive<SendEvent>(obs => obs.ToUnit(m =>
                                                       {
                                                           ...
                                                           if (state.Subscriptions.TryGetValue(eventType,
                                                               out var intrests))
            ...
            public State Update(Type type, Func<ImmutableList<IActorRef>, ImmutableList<IActorRef>> listUpdate)
            {
                if (!Subscriptions.TryGetValue(type, out var list))
                    return this with
                           {
                               Subscriptions =
                               Subscriptions.SetItem(type, listUpdate(ImmutableList<IActorRef>.Empty))
                           };
