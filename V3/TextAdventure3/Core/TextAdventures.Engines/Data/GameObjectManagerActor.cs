...
namespace TextAdventures.Engine.Data
{
    public sealed class GameObjectManagerActor : GameProcess<GameObjectManagerActor.GomState>
    {
        ...
        protected override void Config()
        {
            ...
            Receive<UpdateData>(obs =>
                                    obs.Select(evt =>
                                                   new
                                                   {
                                                       Event = evt,
                                                       Data = ImmutableDictionary<string, GameObject>.Empty
                                                          .AddRange(Observable.Return(evt)
                                                                              .SelectMany(data => data.Event.Data.Select(obj => (obj, data)))
                                                                              .Select(s =>
                                                                               {
                                                                                   var (objData, info) = s;
                                                                                   if (!info.State.GameObjects.TryGetValue(objData.Key,
                                                                                       out var gameObject))
                                                                                       gameObject = new GameObject(objData.Key,
                                                                                           ImmutableDictionary<Type, ComponentObject>.Empty);
                                                                              ...
                                                                              .Select(e =>
                                                                               {
                                                                                   ...
                                                                                   //{
                                                                                   //    Components = ImmutableDictionary<Type, ComponentObject>.Empty
                                                                                   //       .AddRange(objectData.Value
                                                                                   //           .Select(cType => (cType.Key, cType.Value,
                                                                                   //                ImmutableDictionary.GetValueOrDefault(gameObject.Components, cType.Key) ?? 
                                                                                   //                        new ComponentObject(FastReflection.Shared.FastCreateInstance(cType.Key))))
                                                                                   //       .Select(info =>
                                                                                   //            {
                                                                                   //                var (key, _, componentObject) = info;

                                                                                   //                return new KeyValuePair<Type, ComponentObject>(key, componentObject);
                                                                                   //            });
                                                                                                    
                                                                                   //};

                                                                                   return new KeyValuePair<string, GameObject>(objectKey, gameObject);
                                                                               })
                                                                              .ToEnumerable())
                                                   })
                                       .Select(evt => evt.Event.State with {GameObjects = evt.Data}));
        ...
        //                            $"Componet Could not Created {componetDataValue.Key}"));
        ...
        //    return commandObservable.SelectMany(statePair => statePair.State.CommandProcessors
        //                                                              .Lookup(statePair.Event.GetType())
        //                                                              .Select(processorBase
        //                                                                          => (processorBase, statePair)))
        //                            .SelectMany(processor => GetComponents(processor.processorBase.Component,
        //                                                processor.statePair.Event,
        //                                                processor.statePair.State.GameObjects)
        //                                           .Select(component => (
        //                                                       component, processor.processorBase,
        //                                                       processor.statePair.Event)))
        //                            .Where(
        //                                 processor => processor.processorBase.CanProcess(processor.component.componet))
        //                            .ToUnit(processor => processor.processorBase.Run(Game, processor.component.componet,
        //                                        processor.component.obj, processor.Event))
        //                            .SubscribeWithStatus();
        //}
