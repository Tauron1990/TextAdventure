...
namespace TextAdventures.Engine.Internal.Actor
{
    public sealed class ProjectionManagerActor : ExposedReceiveActor
    {
        ...
        private readonly        EventStoreReader  _eventStoreReader            = new(Context.System);

        private readonly GameProjection                  _gameProjection = new();
        ...
        private void ConstructProjections(StartGame obj)
        {
            ...
            try
            {
                Self.Tell(NewProjection.Create<RoomId, RoomProjection, RoomQueryHandler, RoomQueryBase>(new RoomQueryHandler(_gameProjection.Rooms), "Room", disposable => _disposable.Add(disposable),
                                                                                                        eventBuilder =>
                                                                                                        {
                                                                                                            eventBuilder
                ...
                Self.Tell(NewProjection.Create<GameActorId, GameActorProjection, GameActorQueryHandler, GameActorQueryBase>(new GameActorQueryHandler(_gameProjection.GameActors), "GameActor", disposable => _disposable.Add(disposable),
                                                                                                                            eventBuilder =>
                                                                                                                            {
                                                                                                                                eventBuilder
